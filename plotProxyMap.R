...
load(file.path(datDir, 'MS_results_plusNull_complete.RData')) # proxies used for MS/BS
climateVar = 'T'
...
allDF = data.frame(name = names, lat = lats, lon = lons, archive = archives, proxy = proxies)
#allDF = rbind(allDF, allDF2)
...
ggplot() + borders("world", colour="black") + geom_point(aes(x = lons, y = lats, fill = archives, shape = archives)) +
  scale_fill_manual(values = col_m)
## Moisture
sss = c(seq(-90,90), seq(90,-90, by = -1))
boundcirc = data.frame(y = sss, x = c(rep(-180, length.out = length(sss)/2), rep(180, length.out=length(sss)/2)))
baseMap(c(-180,180), c(-90,90),map.type='line',global=T,projection='mollweide',restrict.map.range=F) + 
  geom_path(aes(x = boundcirc$x, y = sss)) +
#ggplot() +
  geom_point(aes(x = lons[which(archives == 'lake sediment')], y = lats[which(archives == 'lake sediment')], fill = 'lake sediment'), shape = 22, size = 4) +
  geom_point(aes(x = lons[which(archives == 'marine sediment')], y = lats[which(archives == 'marine sediment')], fill = 'marine sediment'),  shape = 21, size = 4) +
  geom_point(aes(x = lons[which(archives == 'speleothem')], y = lats[which(archives == 'speleothem')], fill = 'speleothem'), shape = 24, size = 4) +
  #geom_point(aes(x = lons[which(archives == 'glacier ice')], y = lats[which(archives == 'glacier ice')]), color='white', size = 4) +
  #geom_point(aes(x = lons[which(archives == 'midden')], y = lats[which(archives == 'midden')], fill = 'midden'), shape = 25, size = 4) +
  geom_point(aes(x = lons[which(archives == 'ice')], y = lats[which(archives == 'ice')], fill = 'ice'), shape = 21, size = 4) +
  geom_point(aes(x = lons[which(archives == 'peat')], y = lats[which(archives == 'peat')], fill = 'peat'), shape =22, size = 3) +
  #geom_point(aes(x = lons[which(archives == 'wood')], y = lats[which(archives == 'wood')], fill = 'wood'), shape = 23, size = 4) +
  #geom_point(aes(x = lons[which(archives == 'pore ice')], y = lats[which(archives == 'pore ice')], fill = 'pore ice'), shape = 23, size = 4) +
  scale_fill_manual(name = '', values = c('marine sediment' = col_m[1],'lake sediment' = col_m[2],'speleothem' = col_m[4],
                                           'ice' = col_m[6],'peat' = col_m[7]), 
                     #breaks = c('wet MS event','wet BS event','wet EX event','dry MS event','dry BS event','dry EX event','no event'),
                     guide = guide_legend(override.aes = list(shape = c(21, 22, 21, 22, 24), fill = col_m[c(6,2,1,7,4)],
                                                              color = rep('black',5)))) +
  theme_bw() + xlab('Longitude') + ylab('Latitude') +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlim(-180, 180) + ylim(-90, 90) +
  ggtitle('Moisture proxies')
ggsave(file.path(figDir, 'moistureProxyMap_v1.png'))

## Temperature
#archives[length(archives) + 1] = 'lagoon sediment'
#lats[length(lats)] = 63
#lons[length(lons)] = 10
sss = c(seq(-90,90), seq(90,-90, by = -1))
boundcirc = data.frame(y = sss, x = c(rep(-180, length.out = length(sss)/2), rep(180, length.out=length(sss)/2)))
baseMap(c(-180,180), c(-90,90),map.type='line',global=T,projection='mollweide',restrict.map.range=F) + 
  geom_path(aes(x = boundcirc$x, y = sss)) +
  #ggplot() +
  geom_point(aes(x = lons[which(archives == 'midden')], y = lats[which(archives == 'midden')], fill = 'midden'), shape = 23, size = 4) +
  geom_point(aes(x = lons[which(archives == 'marine sediment')], y = lats[which(archives == 'marine sediment')], fill = 'marine sediment'),  shape = 21, size = 4) +
  geom_point(aes(x = lons[which(archives == 'lake sediment')], y = lats[which(archives == 'lake sediment')], fill = 'lake sediment'), shape = 22, size = 4) +
  geom_point(aes(x = lons[which(archives == 'speleothem')], y = lats[which(archives == 'speleothem')], fill = 'speleothem'), shape = 24, size = 4) +
  #geom_point(aes(x = lons[which(archives == 'glacier ice')], y = lats[which(archives == 'glacier ice')]), color='white', size = 4) +
  #geom_point(aes(x = lons[which(archives == 'bog')], y = lats[which(archives == 'bog')], fill = 'bog'), shape = 25, size = 4) +
  geom_point(aes(x = lons[which(archives == 'ice')], y = lats[which(archives == 'ice')], fill = 'ice'), shape = 21, size = 4) +
  geom_point(aes(x = lons[which(archives == 'peat')], y = lats[which(archives == 'peat')], fill = 'peat'), shape =22, size = 4) +
  geom_point(aes(x = lons[which(archives == 'wood')], y = lats[which(archives == 'wood')], fill = 'wood'), shape = 24, size = 4) +
  geom_point(aes(x = lons[which(archives == 'pore ice')], y = lats[which(archives == 'pore ice')], fill = 'pore ice'), shape = 23, size = 4) +
  scale_fill_manual(name = '', values = c('midden' = col_m[3],'marine sediment' = col_m[1],'lake sediment' = col_m[2],'wood' = col_m[8],'speleothem' = col_m[4],
                                          'ice' = col_m[6],'pore ice'=col_m[9],'peat' = col_m[7]), 
                    #breaks = c('wet MS event','wet BS event','wet EX event','dry MS event','dry BS event','dry EX event','no event'),
                    guide = guide_legend(override.aes = list(shape = c(21, 22, 21, 23, 22, 23, 24, 24), fill = col_m[c(6,2,1,3,7,9,4,8)],
                                                             color = rep('black',8)))) +
  theme_bw() + xlab('Longitude') + ylab('Latitude') +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlim(-180, 180) + ylim(-90, 90) +
  ggtitle('Temperature proxies')
ggsave(file.path(figDir, 'temperatureProxyMap_v2.png'))

# Sum these categories for temperature and moisture
print(paste('wood:', length(archives[which(archives == 'wood')])))
print(paste('marine sediment:', length(archives[which(archives == 'marine sediment')])))
print(paste('lake sediment:', length(archives[which(archives == 'lake sediment')])))
...
archs = c('wood','marine sediment','lake sediment','speleothem','pore ice','ice','midden','peat',
          'records','sites')
num_archs = c(length(archives[which(archives == 'wood')]),length(archives[which(archives == 'marine sediment')]),
              length(archives[which(archives == 'lake sediment')]),length(archives[which(archives == 'speleothem')]),
              length(archives[which(archives == 'pore ice')]),length(archives[which(archives == 'ice')]),
              length(archives[which(archives == 'midden')]),length(archives[which(archives == 'peat')]),
...
num_archs2 = c(length(archives2[which(archives2 == 'wood')]),length(archives2[which(archives2 == 'marine sediment')]),
              length(archives2[which(archives2 == 'lake sediment')]),length(archives2[which(archives2 == 'speleothem')]),
              length(archives2[which(archives2 == 'pore ice')]),length(archives2[which(archives2 == 'ice')]),
              length(archives2[which(archives2 == 'midden')]),length(archives2[which(archives2 == 'peat')]),
              length(archives2),length(unique(names)))
rec_df2 = data.frame(archs, num_archs2)
if (climateVar == 'T') {
  #write.csv(rec_df, file.path(figDir, 'temperature_records.csv'))
  write.csv(rec_df2, file.path(figDir, 'temperature_records_unique.csv'))
} else {
  #write.csv(rec_df, file.path(figDir, 'moisture_records.csv'))
