...
output_destination = "/projects/pd_lab/sha59/4ka/Stability_test/J/" #Use dir for the same folder as the inputs. 
...
det = F #detrend records before running the analyses?

# Excursion

event_yr = 3800 # Set the event year (select any year, this will change)
event_window = 600 #(event_yr +/- event_window/2) as the event window. Was 600.
ref_window = 500  # Set the duration of the reference windows. Was 500. 
...
maxDiff = 1000    # min segment length between change points. Was 1000. 
...
eventWindow = 199.9  # total event window = eventYr +/- eventWindow. Was 199.9
