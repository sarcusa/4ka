...
## Map the results of broken stick analysis on a global grid. Plot the number of events occurring within
...
load('RData/BS_results.RData')
rm(breakPts, lat, lon, recordIndex, stdErr)
...
# Grid that will contain fraction of passes
...
ggplot() + borders("world", colour="gray50", fill="gray50") + 
  geom_point(aes(x=longitude[locs[,1]], y=latitude[locs[,2]], color=factor(numEvents)),alpha = .3,size=0.5) +
  scale_color_brewer(name = 'Number\nof events', palette = 'RdYlBu', direction = -1) + 
  geom_point(aes(x = allTsLon, y = allTsLat), color = 'black', size = 1) +
...
ggplot() + borders("world", colour="gray50", fill="gray50") + 
  geom_point(aes(x=longitude[locs[,1]], y=latitude[locs[,2]], color=factor(numEvents)),alpha = .8,size=3) +
  geom_point(aes(x = allTsLon, y = allTsLat), color = 'black', size = 1) +
  ggtitle(paste0('Number of ', eventYr/1000,'+/-',eventWindow/1000, 'ka events')) +
  xlim(-180, 180) + ylim(-90, 90) +
  ylab('Latitude') + xlab('Longitude')
